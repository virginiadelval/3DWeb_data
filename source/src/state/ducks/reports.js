...
const fetchParcelData = async (smp) => {
  const response = await fetch(getParcelBySmp(smp))
  return response.json()
}

const fetchBuildableData = async (smp) => {
  const response = await fetch(getBuildable(smp))
  return response.json()
}

const fetchImages = async (smp) => {
  const response = await fetch(getPhotoData(smp))
  const text = await response.text()
  return JSON.parse(text.slice(1, -1)).length > 0
}

const getData = createAsyncThunk(
  'report/getData',
  async (smp, { getState }) => {
    const { direccion, superficie_total } = await fetchParcelData(smp)

    const buildableData = await fetchBuildableData(smp)

    const isParcelaEnMicrocentro = getState().uses.isParcelaEnMicrocentro

    const hasParcelaEnMicrocentro =
      isParcelaEnMicrocentro === null
        ? await fetch(getParcelasEnMicrocentro(smp)).then((res) =>
            res.json().then((data) => data.in)
          )
        : isParcelaEnMicrocentro

    const constitucionEstadoParcelario = getState().basicData.data
      .constitucionEstadoParcelario

    const hasImage = await fetchImages(smp)
    ...
    const {
      ...
      altura_max,
      distrito_especial,
      fot: { fot_medianera: medianera },
      tipica,
      parcelas_linderas: { aph_linderas },
      catalogacion: { proteccion, denominacion, catalogacion },
      plusvalia: { incidencia_uva, alicuota, distrito_cpu },
      link_imagen: { croquis_parcela, perimetro_manzana, plano_indice },
      subzona,
      manzanas_atipicas: { disposicio, pdf }
    } = buildableData

    const afectaciones = await fetch(getAffectations(smp)).then((response) =>
      response.json()
    )
    ...
    const { usos } = await fetch(getUses(smp)).then((response) =>
      response.json()
    )
    ...
    perfilEdificableImage =
      perfilEdificableImage && `images/${perfilEdificableImage}`

    let distritoEspecial = distrito_especial
      ?.map(({ distrito_especifico }) => distrito_especifico)
      .filter((value) => (value?.length ?? '') > 0)
      .join(', ')
    distritoEspecial = distritoEspecial?.length ? distritoEspecial : null

    let distritoAPH = distrito_especial
      ?.filter(({ distrito_agrupado }) => distrito_agrupado === 'APH')
      ?.map(({ distrito_especifico }) => distrito_especifico)
      ?.join(', ')
    distritoAPH = distritoAPH?.length ? distritoAPH : null
    ...
    const sections = [
        ...
        dataList: [
          ...
          },
          {
            ...
            value: superficie_total
              ? `${Number.parseFloat(superficie_total).toLocaleString(
                  'es-AR'
                )} m²`
              : ''
          },
      ...
      },
      {
        ...
        dataList: [
          ...
          {
            name: 'Subzona',
            value: subzona ? `${subzona} (Ley 6361 Anexo II)` : null
          },
          {
          ...
          },
          {
            ...
            value: atipica.length ? atipica : null
          },
          {
            name: 'Afectaciones',
            value: afectacionesText?.length ? afectacionesText : null
          }
        ]
      },
      {
            ...
            value: usoCatalogado
              ? [
                  'Se deberá consultar al Organismo Competente a través de una ',
                  {
                    ...
                    textReport:
                      'https://www.buenosaires.gob.ar/tramites/consulta-de-usos',
                    type: 'LINK'
                  },
      ...
      },
      {
        ...
        dataList: [
          ...
          {
            name: 'Denominación',
            value: denominacion?.length ? denominacion : null
          },
          {
            name: 'Catalogación',
            value: catalogacion?.length ? catalogacion : null
          },
          {
            name: 'Protección',
            value: proteccion?.length ? proteccion : null
          },
          {
            name: '',
            value: usoCatalogado
          ...
          },
          {
            ...
            value: aph_linderas ? adyacenteCatalogado.reportText : null
          }
        ]
      }
    ]

    hasParcelaEnMicrocentro &&
      sections.push({
    ...
    constitucionEstadoParcelario?.data.length > 0 &&
      sections.push({
        title: 'Constitución Estado Parcelario (CEP)',
        dataList: [
          {
            name: 'Estado parcelario constituido',
            value:
              constitucionEstadoParcelario?.data[0]
                .estado_parcelario_constituido
          },
          {
            name: 'Fecha de constitución',
            value: constitucionEstadoParcelario?.data[0].fecha_de_constitucion
          },
          {
            name: 'Tipo de construcción',
            value: constitucionEstadoParcelario?.data[0].tipo_de_construccion
          },
          {
            name: 'Vencimiento de construcción',
            value:
              constitucionEstadoParcelario?.data[0].vencimiento_de_construccion
          }
        ]
      })
